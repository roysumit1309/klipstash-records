...
/**
 * Does this look like a whole Google client id?
 *
 * The old check was `endsWith(".apps.googleusercontent.com")`, which a
 * truncated paste passes: the damage is always in the middle, and the
 * tail is exactly the part that survives. A real id is a project
 * number, a dash, and a long random segment - so check the shape, not
 * the suffix.
 */
export function looksLikeClientId(id) {
  return /^\d{6,}-[a-z0-9]{20,}\.apps\.googleusercontent\.com$/i.test(String(id).trim());
}

/**
 * A client id passed in the URL, as `?client_id=...`.
 *
 * Typing or pasting a seventy-two character string on a phone is how
 * this went wrong repeatedly - a truncated paste produces
 * `invalid_client`, which names nothing useful. A link carries it
 * exactly.
 */
export function clientIdFromUrl() {
  const value = new URLSearchParams(location.search).get("client_id");
  return value && looksLikeClientId(value) ? value.trim() : null;
}
...
function describeAuthError(code) {
  switch (code) {
    ...
    case "invalid_client":
      // The id is echoed back deliberately. A project ends up holding
      // Desktop, Android and Web clients whose ids share a project
      // prefix and differ only after the dash, so "that id is wrong"
      // without saying WHICH id was tried leaves the user comparing
      // strings they cannot see.
      return `Google did not recognise this client id: ${storedClientId()} - it must be the WEB client from your Google Cloud project. Desktop and Android clients do not work in a browser.`;
    default:
